...
import { ICON_NETWORK_CONFIG } from "@/config";
...
const useProvideAuth = (auth) => {
  ...
  const onConnectHook = async () => {
    ...
    if (loggedIn && auth.provider.chainId !== ICON_NETWORK_CONFIG.chainId) {
      await switchNetwork();
    }
    setUser(info);
    setLoading(false);
  ...
  const switchNetwork = async () => {
    try {
      await auth.provider.request({
        ...
        params: [{ chainId: ICON_NETWORK_CONFIG.chainId }],
      });
    } catch (error) {
      if (error.code === 4902) {
        try {
    ...
    auth.init().then(async () => {
      try {
        ...
        if (loggedIn) {
          const info = await auth.getUser();
          setIsLoggedIn(true);
          if (auth.provider.chainId !== ICON_NETWORK_CONFIG.chainId) {
            await switchNetwork();
          }
          setUser(info);
        } else {
